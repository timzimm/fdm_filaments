...
def init_wavefunction(
    ...
    source = lambda R: rho(R) / a_fit
    source.name = rho.name
    V = AxialSymmetricPotential(source, rho.R999)
    eigenstate_args = {
        "for_name": [a_fit, V.name, V(rho.R99), N],
        "for_compute": [a_fit, V, V(rho.R99), N],
    }
    name_lib, eigenstate_lib = load_or_compute_model(
        load_if_cached,
        cache_dir,
    ...
    _, wavefunction_params = load_or_compute_model(
        load_if_cached,
        cache_dir,
...
init_routines = {
    "wkb": psi.init_wavefunction_params_wkb,
